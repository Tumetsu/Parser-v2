...
def orthonormal_initializer(input_size, output_size):
  ...
  if not tf.compat.v1.get_variable_scope().reuse:
    print(tf.compat.v1.get_variable_scope().name,file=sys.stderr)
...
def linear(inputs, output_size, n_splits=1, add_bias=True, initializer=None, moving_params=None):
  ...
  output_shape = []
  shape = tf.shape(input=all_inputs)
  ...
  with tf.compat.v1.variable_scope('Linear'):
    # Get the matrix
    if initializer is None and not tf.compat.v1.get_variable_scope().reuse:
      mat = orthonormal_initializer(input_size, output_size//n_splits)
      mat = np.concatenate([mat]*n_splits, axis=1)
      initializer = tf.compat.v1.constant_initializer(mat)
    matrix = tf.compat.v1.get_variable('Weights', [input_size, output_size], initializer=initializer)
    ...
    # Get the bias
    if add_bias:
      bias = tf.compat.v1.get_variable('Biases', [output_size], initializer=tf.compat.v1.zeros_initializer())
      if moving_params is not None:
...
def bilinear(inputs1, inputs2, output_size, n_splits=1, add_bias1=True, add_bias2=True, initializer=None, moving_params=None):
  ...
  inputs1_bucket_size = tf.shape(input=all_inputs1)[-2]
  ...
  inputs2_bucket_size = tf.shape(input=all_inputs2)[-2]
  
  # Prepare the output
  output_size *= n_splits
  output_shape = []
  shape1 = tf.shape(input=all_inputs1)
  ...
  if add_bias1:
    bias1 = tf.ones(tf.stack([tf.shape(input=all_inputs1)[0], inputs1_bucket_size, 1]))
    all_inputs1 = tf.concat([all_inputs1, bias1], 2)
    inputs1_size += 1
  all_inputs2 = tf.reshape(all_inputs2, tf.stack([-1, inputs2_bucket_size, inputs2_size]))
  if add_bias2:
    bias2 = tf.ones(tf.stack([tf.shape(input=all_inputs2)[0], inputs2_bucket_size, 1]))
    all_inputs2 = tf.concat([all_inputs2, bias2], 2)
    inputs2_size += 1
  with tf.compat.v1.variable_scope('Bilinear'):
    # Get the matrix
    if initializer is None and tf.compat.v1.get_variable_scope().reuse is None:
      mat = orthonormal_initializer(inputs1_size, inputs2_size)[:,None,:]
      mat = np.concatenate([mat]*output_size, axis=1)
    weights = tf.compat.v1.get_variable('Weights', [inputs1_size, output_size, inputs2_size], initializer=initializer)
    if moving_params is not None:
...
def convolutional(inputs, window_size, output_size, n_splits=1, add_bias=True, initializer=None, moving_params=None):
  ...
  all_inputs = tf.concat(inputs, n_dims-1)
  input_size = all_inputs.get_shape().as_list()[-1]
  bucket_size = tf.shape(input=all_inputs)[-2]
  ...
  with tf.compat.v1.variable_scope('Convolutional'):
    ...
    if initializer is None and not tf.compat.v1.get_variable_scope().reuse:
      ...
      initializer = tf.compat.v1.constant_initializer(mat)
    matrix = tf.compat.v1.get_variable('Weights', [window_size, input_size, output_size], initializer=initializer)
    if moving_params is not None:
      matrix = moving_params.average(matrix)
    else:
      tf.compat.v1.add_to_collection('Weights', matrix)
    ...
    # Do the multiplication
    conv = tf.nn.conv1d(input=all_inputs, filters=matrix, stride=1, padding='SAME') + bias
...
def random_mask(prob, mask_shape, dtype=tf.float32):
  ...
  return tf.compat.v1.where(rand < prob, ones, zeros)

#===============================================================
def random_where(prob, success, fail, keep_dims=None): 
  """ """
  
  mask_shape = tf.shape(input=success)
